package com.roy.app.mvptemplate.presentation.view.util;
...
//            }
    ...
    /**
     * 用于一个Activity切换多个fragment时要改变其状态栏的颜色和高度
     * 支持 4.4 以上版本的 MIUI 和 Flyme，以及 5.0 以上版本的其他 Android。
     *
     * @param activity        当前要设置的Activity
     * @param color           状态栏颜色
     * @param statusBarHeight 状态栏高度
     */
    public static void setStatusBarColor(Activity activity, int color, int statusBarHeight) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                if (activity.getWindow().getStatusBarColor() != Color.TRANSPARENT) {
                    //此时说明不支持更改状态栏Icon和字体颜色，translucent()已设置过背景不需要再次设置
                    return;
                }
                //5.0 以上直接设置状态栏颜色
     ...
     * 支持 4.4 以上版本。
     *
     * @param activity 当前要设置activity
     * @param color    状态栏颜色
     */
    public static void setStatusBarColor(Activity activity, int color) {
        setStatusBarColor(activity,color,getStatusBarHeight(activity));
...
//            MIUISetStatusBarLightMode(window, light);
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (DeviceUtil.isTablet(context)
                && sStatusbarHeight > dp2px(STATUS_BAR_DEFAULT_HEIGHT_DP)) {
            //状态栏高度大于25dp的平板，状态栏通常在下方
            sStatusbarHeight = 0;
        } else {
            if (sStatusbarHeight <= 0
                    || sStatusbarHeight > dp2px(STATUS_BAR_DEFAULT_HEIGHT_DP * 2)) {
